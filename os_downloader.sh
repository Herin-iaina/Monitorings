...
download_file() {
    ...
    # Utiliser l'endpoint /installers/ pour les scripts .sh
    if [[ "$filename" == *.sh ]]; then
        local url="${SERVER_URL}/installers/${encoded_filename}"
    else
    ...
    if curl -L -o "$destination" "$url" 2>>"$LOG_FILE"; then
        ...
        if [[ "$filename" == *.sh ]]; then
            chmod +x "$destination"
            log "INFO" "Droit d'exécution ajouté à $filename"
        fi
...
download_all_files() {
    # Télécharger les scripts d'installation depuis /installers/
    local scripts=("os_downloader.sh" "os_installer.sh")
    for script in "${scripts[@]}"; do
        if ! download_file "$script"; then
            log "ERROR" "Échec du téléchargement du script: $script"
            return 1
        fi
    done
    
    # Obtenir la liste des autres fichiers depuis /files/
    local files_json
    files_json=$(curl -s "${SERVER_URL}/files")
    
    if [ $? -ne 0 ]; then
        log "WARNING" "Erreur lors de la récupération de la liste des fichiers depuis /files/, continuation..."
        return 0
    fi
    ...
    if [ -n "$files" ]; then
        log "INFO" "Téléchargement des fichiers depuis /files/..."
        ...
        if ! $success; then
            log "ERROR" "Échec du téléchargement de certains fichiers"
            return 1
        fi
    else
        log "INFO" "Aucun fichier supplémentaire trouvé dans /files/"
    fi
...
main() {
    ...
    # (Suppression de l'exécution automatique de os_installer.sh)
